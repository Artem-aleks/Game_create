...
class Window:
        ...
        for x in range(size[0]):
            for y in range(size[1]):
                if pix[x, y] != (0, 0, 0, 255):
                    Window.d1_point(self, x, y, self.window.get_surface(), (255,255,255))
    ...
    def run(self):
        ...
        print("Ракета может летать пока она не получит кретическое повреждение")
        print("Имейте терпение!Игра прогружается довольно долго")
        print("Хорошей игры!!!")
        while running:
            ...
            for event in events:
                if event.type == sdl2.SDL_QUIT:
                    ...
                    if event.key.keysym.sym == sdl2.SDLK_SPACE:
                        Window.fill_Window(self, (192, 192, 192))
                        Window.draw_raketa(self, k, s)
                        sp1 = Window.draw_raketa(self, 0, z)
                        k = 0
                        Window.rectangle(self, 200, 1 + i, 100, s)
                        Window.rectangle(self, 900, 1 + m, 100, s)
                        Window.rectangle(self, 1500, 1 + q, 100, s)
                    ...
                    if event.key.keysym.sym == sdl2.SDLK_LEFT:
                        ...
                        if w == 0:
                            ...
                            Window.draw_raketa(self, k, s)
                            k -= 20
                            sp = Window.draw_raketa(self, k,z)
                            Window.rectangle(self, 200, 1 + i, 100, s)
                            i += 20
                            sp1 = Window.rectangle(self, 200, 1 + i, 100, z)
                            Window.rectangle(self, 900, 1 + m, 100, s)
                            m += 20
                            sp2 = Window.rectangle(self, 900, 1 + m, 100, z)
                            Window.rectangle(self, 1500, 1 + q, 100, s)
                            q += 20
                            sp3 = Window.rectangle(self, 1500, 1 + q, 100, z)
                            for r in sp:
                                for j in range(len(sp1)):
                                    if r == sp1[j] or r == sp2[j] or r == sp3[j]:
                                        Window.game_over(self)
                        if w == 1:
                            sp = []
                            sp1 = []
                            sp2 = []
...
def main():
    window = Window((1900, 800), "Space Debris")
    pg.init()
